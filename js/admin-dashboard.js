(function(){
  ...
  const sectionTitles={
    dashboard:"Dashboard",analytics:"Website Analytics",products:"Products",orders:"Orders",returns:"Returns & Refunds",customers:"Customers",abandoned:"Abandoned Carts",reviews:"Reviews",
    messages:"Messages",subscribers:"Subscribers",discounts:"Discounts",approvals:"Approvals",activity:"Activity Log",notifications:"Notifications",popup:"Message Popup",settings:"Website Settings",health:"System Health",admins:"Admin Users"
  };
  ...
  function showSection(name,pushHash=true){
    ...
    if(name==="analytics")window.LZAdminAnalytics?.load?.();
  ...
  async function saveDiscount(event){event.preventDefault();if(!window.LZAdminAccess?.requirePrepare("discounts","discounts"))return;const id=valueOf("discount-id"),payload={code:valueOf("discount-code").toUpperCase(),discount_type:valueOf("discount-type"),discount_value:Number(valueOf("discount-value")),min_order:Number(valueOf("discount-min-order")||0),usage_limit:valueOf("discount-usage-limit")?Number(valueOf("discount-usage-limit")):null,expires_at:valueOf("discount-expiry")?new Date(valueOf("discount-expiry")).toISOString():null,active:byId("discount-active").checked},msg=byId("discount-form-message");msg.textContent="Saving…";const route=await window.LZAdminAccess.routeChange("discounts","discount_save",`${id?"Edit":"Create"} coupon ${payload.code}`,{id:id||null,data:payload});if(route.mode==="queued"){msg.textContent="Submitted for approval. The coupon is not live yet.";msg.style.color="#8a6c3f";byId("discount-form").hidden=true;return;}if(route.mode!=="direct")return;const result=id?await safeQuery(supabaseClient.from("discounts").update(payload).eq("id",id)):await safeQuery(supabaseClient.from("discounts").insert(payload));if(result.error){msg.textContent=result.error.message;msg.style.color="#a6473d";return;}msg.textContent="Saved.";msg.style.color="#35725d";byId("discount-form").hidden=true;loadDiscounts();}
  async function deleteDiscount(id){if(!window.LZAdminAccess?.requirePrepare("discounts","discounts"))return;if(!confirm("Delete this coupon permanently?"))return;const row=discountsCache.find(x=>String(x.id)===String(id));const route=await window.LZAdminAccess.routeChange("discounts","discount_delete",`Delete coupon ${row?.code||id}`,{id});if(route.mode==="queued"){alert("Coupon deletion submitted for approval.");return;}if(route.mode!=="direct")return;const r=await safeQuery(supabaseClient.from("discounts").delete().eq("id",id));if(r.error){alert(r.error.message);return;}loadDiscounts();}
  ...
  async function savePopupSettings(event){event?.preventDefault?.();if(!window.LZAdminAccess?.requirePrepare("popup","message popup"))return;
    ...
    const route=await window.LZAdminAccess.routeChange("popup","popup_update",`${payload.popup_enabled?"Publish":"Save"} promotional popup: ${payload.popup_title||"campaign"}`,{updates:payload});
    if(route.mode==="queued"){msg.textContent="Submitted for approval. The current customer popup has not changed.";msg.style.color="#8a6c3f";return;}
  ...
  async function uploadPopupImage(file){if(!window.LZAdminAccess?.requirePrepare("popup","popup images"))return;
  ...
  async function saveFaq(){if(!window.LZAdminAccess?.requirePrepare("settings","FAQs"))return;const id=valueOf("faq-id"),msg=byId("faq-message"),payload={question:valueOf("faq-question"),answer:valueOf("faq-answer"),sort_order:Number(valueOf("faq-order")||0),active:byId("faq-active").checked,updated_at:new Date().toISOString()};if(!payload.question||!payload.answer){msg.textContent="Question and answer are required.";return;}msg.textContent="Saving…";const route=await window.LZAdminAccess.routeChange("settings","faq_save",`${id?"Edit":"Create"} FAQ: ${payload.question}`,{id:id||null,data:payload});if(route.mode==="queued"){msg.textContent="Submitted for approval. The FAQ has not changed yet.";msg.style.color="#8a6c3f";byId("faq-editor").hidden=true;return;}if(route.mode!=="direct")return;const r=id?await safeQuery(supabaseClient.from("site_faqs").update(payload).eq("id",id)):await safeQuery(supabaseClient.from("site_faqs").insert(payload));if(r.error){msg.textContent=r.error.message;msg.style.color="#a6473d";return;}byId("faq-editor").hidden=true;loadFaqs();}
  async function deleteFaq(id){if(!window.LZAdminAccess?.requirePrepare("settings","FAQs"))return;if(!confirm("Delete this FAQ?"))return;const row=faqsCache.find(x=>String(x.id)===String(id));const route=await window.LZAdminAccess.routeChange("settings","faq_delete",`Delete FAQ: ${row?.question||id}`,{id});if(route.mode==="queued"){alert("FAQ deletion submitted for approval.");return;}if(route.mode!=="direct")return;const r=await safeQuery(supabaseClient.from("site_faqs").delete().eq("id",id));if(r.error)alert(r.error.message);else loadFaqs();}
  ...
  async function saveGuide(){if(!window.LZAdminAccess?.requirePrepare("settings","journal guides"))return;const original=valueOf("guide-original-slug"),slug=valueOf("guide-slug").toLowerCase().replace(/[^a-z0-9-]+/g,"-").replace(/^-+|-+$/g,""),msg=byId("guide-message"),payload={slug,title:valueOf("guide-title"),collection:valueOf("guide-collection")||"abayas",summary:valueOf("guide-summary"),sections:collectGuideSections(),sort_order:Number(valueOf("guide-order")||0),active:byId("guide-active").checked,updated_at:new Date().toISOString()};if(!payload.slug||!payload.title||!payload.summary||!payload.sections.length){msg.textContent="Slug, title, summary and at least one complete section are required.";return;}msg.textContent="Saving…";const route=await window.LZAdminAccess.routeChange("settings","guide_save",`${original?"Edit":"Create"} guide: ${payload.title}`,{original_slug:original||null,data:payload});if(route.mode==="queued"){msg.textContent="Submitted for approval. The journal has not changed yet.";msg.style.color="#8a6c3f";byId("guide-editor").hidden=true;return;}if(route.mode!=="direct")return;let r;if(original&&original!==slug){r=await safeQuery(supabaseClient.from("journal_articles").insert(payload));if(!r.error)await safeQuery(supabaseClient.from("journal_articles").delete().eq("slug",original));}else r=await safeQuery(supabaseClient.from("journal_articles").upsert(payload));if(r.error){msg.textContent=r.error.message;msg.style.color="#a6473d";return;}byId("guide-editor").hidden=true;loadGuides();}
  async function deleteGuide(slug){if(!window.LZAdminAccess?.requirePrepare("settings","journal guides"))return;if(!confirm(`Delete /journal/${slug}/?`))return;const row=guidesCache.find(x=>x.slug===slug);const route=await window.LZAdminAccess.routeChange("settings","guide_delete",`Delete guide: ${row?.title||slug}`,{slug});if(route.mode==="queued"){alert("Guide deletion submitted for approval.");return;}if(route.mode!=="direct")return;const r=await safeQuery(supabaseClient.from("journal_articles").delete().eq("slug",slug));if(r.error)alert(r.error.message);else loadGuides();}


  const adminRoleLabels={owner:"Owner",managing_director:"Managing Director",admin:"Admin",viewer:"Viewer"};
  const accessKeys=["analytics","products","orders","returns","customers","abandoned","reviews","messages","subscribers","discounts","approvals","activity","notifications","popup","settings","health"];
  ...
  function applyStaffRolePreset(role,editing=false){
    const apply=byId("staff-can-apply"),manage=byId("staff-can-manage-admins"),approval=byId("staff-requires-approval");
    if(role==="owner"){apply.checked=true;apply.disabled=true;manage.checked=true;manage.disabled=true;approval.checked=false;approval.disabled=true;document.querySelectorAll("[data-staff-permission]").forEach(x=>{x.checked=true;x.disabled=true;});}
    else if(role==="viewer"){apply.checked=false;apply.disabled=true;manage.checked=false;manage.disabled=true;approval.checked=false;approval.disabled=true;document.querySelectorAll("[data-staff-permission]").forEach(x=>x.disabled=false);}
    else{apply.disabled=false;manage.disabled=false;approval.disabled=false;document.querySelectorAll("[data-staff-permission]").forEach(x=>x.disabled=false);if(!editing&&role==="managing_director"){apply.checked=true;manage.checked=true;approval.checked=false;document.querySelectorAll("[data-staff-permission]").forEach(x=>x.checked=true);}if(!editing&&role==="admin"){const abandoned=document.querySelector('[data-staff-permission="abandoned"]'),health=document.querySelector('[data-staff-permission="health"]');if(abandoned)abandoned.checked=true;if(health)health.checked=false;}}
  }
  function renderMyAdminAccount(){const p=window.LZAdminAccess?.profile||{},name=p.full_name||p.email||"Admin",initial=(name.trim()[0]||"A").toUpperCase();if(byId("admin-self-avatar"))byId("admin-self-avatar").textContent=initial;if(byId("admin-self-name"))byId("admin-self-name").textContent=name;if(byId("admin-self-meta"))byId("admin-self-meta").textContent=[p.email,p.phone].filter(Boolean).join(" · ")||"—";if(byId("admin-self-role"))byId("admin-self-role").textContent=window.LZAdminAccess?.roleLabel(p.role)||"Admin";if(byId("admin-self-change"))byId("admin-self-change").textContent=(p.role==="owner")?"Changes enabled":(!p.can_apply_changes?"View only":(p.requires_approval?"Approval required":"Changes enabled"));}
  ...
  function loadCurrentSection(force=false){const section=(location.hash||"#dashboard").slice(1);if(section==="dashboard")loadDashboard(force);else if(section==="analytics")window.LZAdminAnalytics?.load?.(force);else if(section==="customers")loadCustomers(force);else if(section==="discounts")loadDiscounts();else if(section==="popup")loadPopupSettings();else if(section==="settings")loadSettings();else if(section==="admins")loadAdminUsers();}
