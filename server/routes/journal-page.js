...
const e=C.escape;
const safeGuideHtml=require('../lib/guide-html');
exports.handler=async event=>{
 const path=event.path || '/journal/';
 const match=path.match(/^\/journal(?:\/([^/]+))?\/?$/);
 if(!match) return R.notFound();
 const [articles,catalog]=await Promise.all([Journal.list(),Catalog.all().catch(()=>[])]);
 ...
 const recommendationCollection=article && Collections.get(article.collection);
 const matching=recommendationCollection?catalog.filter(p=>p.inStock && p.price>0 && Collections.matches(p,recommendationCollection)):[];
 const recommendations=(matching.length?matching:catalog.filter(p=>p.inStock && p.price>0)).slice(0,3);
 const productSection=article && recommendations.length?`<section class="journal-products"><h2>From the current collection</h2><p>Compare the photographs, available sizes and current product details.</p><div class="home-product-grid collection-grid">${recommendations.map(p=>R.card(p,99)).join('')}</div></section>`:'';
 const content=`<main id="main-content"><header class="page-header"><div class="wrap">${R.crumbs(crumbs)}<p class="eyebrow">The Label by Zare journal</p><h1 class="display-2">${e(title)}</h1><p class="lede">${e(description)}</p>${article ? '<p class="journal-meta">By Label by Zare · Buying &amp; styling guidance</p>' : ''}</div></header><section class="section-tight"><div class="wrap">${article ? `<div class="journal-layout"><article class="journal-body">${article.sections.map(([heading,body],i)=>`<section id="guide-${i+1}"><h2>${e(heading)}</h2>${safeGuideHtml(body)}</section>${i===0?productSection:''}`).join('')}<div class="journal-note"><p>Explore the current ${e(collectionName(article).toLowerCase())} edit. Product pages show each piece’s current price, availability and details.</p><a class="link-underline" href="${collectionUrl(article)}">Shop ${e(collectionName(article))}</a></div></article><aside class="journal-toc"><p class="eyebrow">In this guide</p>${article.sections.map(([heading],i)=>`<a href="#guide-${i+1}">${e(heading)}</a>`).join('')}<a href="/support#size-guide">Size guide</a><a href="/support#shipping-returns">Delivery &amp; returns</a><a href="/contact">Ask us a question</a></aside></div><div class="journal-related"><h2 class="display-3">Continue reading</h2><div class="journal-grid" style="margin-top:1.5rem">${cards}</div></div>` : `<div class="journal-grid">${cards}</div>`}</div></section></main>`;
 ...
 return R.response(R.page({title:title+' | Label by Zare',description,path,content,schemas,products:recommendations}));
};
