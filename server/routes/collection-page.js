...
exports.handler=async event=>{
  ...
  try {
    ...
    if(pageNumber>pages || (total===0 && (query.stock || query.edit))) return R.notFound();
    ...
    const description=collection?.description || (search ? 'Search Label by Zare by product name, colour, fabric or collection.' : 'Discover abayas and shawls by Label by Zare. Browse available pieces, compare prices in PKR and order for delivery across Pakistan.');
    ...
    const content=`<main id="main-content"><header class="page-header"><div class="wrap">${R.crumbs(breadcrumbs)}<p class="eyebrow">The Label by Zare wardrobe</p><h1 class="display-2">${e(collection?.heading || name)}</h1><p class="lede">${e(collection?.intro || description)}</p>${R.collectionLinks(collection?.slug)}</div></header><section class="section-tight"><div class="wrap"><form class="catalog-controls" method="get" action="${e(path)}">${searchField}<label>Sort by<select class="select-min" name="sort">${[['featured',search ? 'Most relevant' : 'Recommended'],['new','New arrivals'],['price-asc','Price: low to high'],['price-desc','Price: high to low']].map(([v,t])=>`<option value="${v}"${sort===v?' selected':''}>${t}</option>`).join('')}</select></label><label class="stock-filter"><input type="checkbox" name="stock" value="in"${query.stock==='in'?' checked':''}> In stock only</label>${query.edit ? `<input type="hidden" name="edit" value="${e(query.edit)}">` : ''}<button class="btn btn-outline btn-sm" type="submit">${search?'Search':'Apply'}</button>${filtered ? `<a class="link-underline" href="${e(path)}">Clear filters</a>` : ''}</form><p class="catalog-count" role="status">${total} ${total===1?'piece':'pieces'}${search && q ? ` for “${e(q)}”` : ''}</p><h2 class="sr-only">${e(collection ? "Shop "+collection.name : name)}</h2><div class="home-product-grid collection-grid" id="collection-grid">${visible.length ? visible.map(R.card).join('') : `<div class="home-empty"><p>${empty}</p><a class="link-underline" href="/collections/abayas/">Explore abayas</a></div>`}</div>${pagination}</div></section>${collection ? `<section class="collection-advice section-tight"><div class="wrap advice-layout"><div><p class="eyebrow">A considered choice</p><h2 class="display-3">${e(collection.slug==='abayas'?'Choosing your abaya: fit and fabric':'Choosing your '+collection.name.toLowerCase())}</h2></div><div><p>${e(collection.advice)}</p><h3>Compare the details before ordering</h3><p>Check fabric and care notes, available sizes and what is included in the price. For abayas, compare garment length and sleeves with a piece you already wear comfortably. <a href="/support#size-guide">View the size guide</a>.</p><h3>Explore related collections</h3>${R.collectionLinks(collection.slug)}<p>We deliver across Pakistan, including Karachi, Lahore, Islamabad, Rawalpindi and Faisalabad. Check the current <a href="/support#shipping-returns">delivery and returns information</a> before placing your order.</p><a class="link-underline" href="/journal/${collection.guide}/">Read the ${collection.name.toLowerCase()} guide</a></div></div></section>` : ''}</main>`;
